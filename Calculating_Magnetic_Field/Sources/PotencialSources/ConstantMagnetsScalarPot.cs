...
namespace Calculating_Magnetic_Field
{
    ...
    class ConstantMagnetScalarPot : IResidualIntensitySource, ISource
    {
        int n;
        ...
        List<Bound_Rib> bigRibsForScal = new List<Bound_Rib>(2);

        List<Bound_Rib> ribsForScal;
        ...
        public ConstantMagnetScalarPot(PhysicalField physicalField, Bound_Rectangle rectangle, SimpleDirections direction, double M, int N)
        {
            n = N;
            this.direction = direction;
            thisFigure = rectangle;
            coerciveForce = M;
            ribsForScal = new List<Bound_Rib>(2 * n);
            ...
            switch (direction)
            {
                case SimpleDirections.FromBottomToTop:
                    {
                        bigRibsForScal.Add(new Bound_Rib(new PointD(rectangle.Location.X, rectangle.Location.Y - rectangle.Width), new PointD(rectangle.Location.X + rectangle.Lenth, rectangle.Location.Y - rectangle.Width)));
                        ribsForScal.AddRange(bigRibsForScal[0].GetSubRibs(n));
                        bigRibsForScal.Add(new Bound_Rib(new PointD(rectangle.Location.X + rectangle.Lenth, rectangle.Location.Y), new PointD(rectangle.Location.X, rectangle.Location.Y)));
                        ribsForScal.AddRange(bigRibsForScal[1].GetSubRibs(n));
                ...
                case SimpleDirections.FromLeftToRight:
                    {
                        bigRibsForScal.Add(new Bound_Rib(new PointD(rectangle.Location.X, rectangle.Location.Y), new PointD(rectangle.Location.X, rectangle.Location.Y - rectangle.Width)));
                        ribsForScal.AddRange(bigRibsForScal[0].GetSubRibs(n));
                        bigRibsForScal.Add(new Bound_Rib(new PointD(rectangle.Location.X + rectangle.Lenth, rectangle.Location.Y - rectangle.Width), new PointD(rectangle.Location.X + rectangle.Lenth, rectangle.Location.Y)));
                        ribsForScal.AddRange(bigRibsForScal[1].GetSubRibs(n));
                        /*for (int i = 0; i < n; i++)
                            densities.Add(-standart_field_property * coerciveForce);
                        for (int i = 0; i < n; i++)
        ...
        public Vector2D GetGradientValue(PointD pointM)
        {
            ...
            PointD midP;
            double r;
            for (int j = 0; j < ribsForScal.Count; j++)
            {
                rib = ribsForScal[j];
                midP = new PointD(rib.GetMiddleOfRib());
                r = Math.Sqrt((midP.X - pointM.X) * (midP.X - pointM.X) + (midP.Y - pointM.Y) * (midP.Y - pointM.Y));
                lenth = ribsForScal[j].LengthElement;
                if (r < lenth)
                {

                    if (rib.Classify(pointM) == PointPosition.LEFT)
                    {
                        p1 = new PointD(pointM.X - rib.Normal.CosAlpha * bigRibsForScal[0].LengthElement * coef, pointM.Y - rib.Normal.CosBeta * bigRibsForScal[0].LengthElement * coef);
                        p2 = new PointD(pointM.X - 2 * rib.Normal.CosAlpha * bigRibsForScal[0].LengthElement * coef, pointM.Y - 2 * rib.Normal.CosBeta * bigRibsForScal[0].LengthElement * coef);
                        r = Math.Sqrt((p1.X - pointM.X) * (p1.X - pointM.X) + (p1.Y - pointM.Y) * (p1.Y - pointM.Y));
                        f1.X_component += lenth * densities[j] * ((pointM.X - p1.X) / (r * r));
                        f1.Y_component += lenth * densities[j] * ((pointM.Y - p1.Y) / (r * r));
                        r = Math.Sqrt((p2.X - pointM.X) * (p2.X - pointM.X) + (p2.Y - pointM.Y) * (p2.Y - pointM.Y));
                        f2.X_component += lenth * densities[j] * ((pointM.X - p2.X) / (r * r));
                        f2.Y_component += lenth * densities[j] * ((pointM.Y - p2.Y) / (r * r));
                    }
                ...
                result.X_component += (pointM.X - midP.X) / (r * r) * densities[j] * lenth;
                result.Y_component += (pointM.Y - midP.Y) / (r * r) * densities[j] * lenth;
            }
        ...
        public Vector2D GetInductionValue(PointD pointM)
        {
            ...
            if (thisFigure.IsContaisPoint(pointM))
                result += standart_field_property * coerciveForce * directionVector;
            return result;
        }
        ...
        public double GetScalarPotencialValue(PointD pointM)
        {
            double result = 0;
            for (int j = 0; j < ribsForScal.Count; j++)
                result += Integral_Potencial(ribsForScal[j], pointM, densities[j]);
